...
const deleteLocation = async (req, res) => {
  try {
    const { id } = req.params;
    const location = await Location.findByIdAndDelete({ user_id: id, });
    if (!location) {
...
const updateLocation = async (req, res) => {
  try {
    ...
    const updatedLocationData = { name, address, latitude, longitude,user_id };

    // Update the location with the new data
    const updatedLocation = await Location.findOneAndUpdate({ user_id: id, }, updatedLocationData, { new: true });
